...
import sv.edu.udb.www.cart.ejb.beans.PruebaCarritoCompraLocal;
...
public class ProcesarCompraCarrito extends HttpServlet {
    ...
    PruebaCarritoCompraLocal pruebaCarritoCompra = lookupPruebaCarritoCompraLocal();
    ...
    private PruebaCarritoCompraLocal lookupPruebaCarritoCompraLocal() {
        try {
            Context c = new InitialContext();
            return (PruebaCarritoCompraLocal) c.lookup("java:global/carritoCompra/carritoCompra-ejb/PruebaCarritoCompra!sv.edu.udb.www.cart.ejb.beans.PruebaCarritoCompraLocal");
    ...
    private CarritoDeCompraLocal lookupCarritoDeCompraLocal() {
        try {
            Context c = new InitialContext();
            return (CarritoDeCompraLocal) c.lookup("java:global/carritoCompra/carritoCompra-ejb/CarritoDeCompra!sv.edu.udb.www.cart.ejb.beans.CarritoDeCompraLocal");
        } catch (NamingException ne) {
            Logger.getLogger(getClass().getName()).log(Level.SEVERE, "exception caught", ne);
            throw new RuntimeException(ne);
        }
